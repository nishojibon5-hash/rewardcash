...
export default function Landing() {
  ...
  if (!landing) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        ...
        <p className="text-slate-600 mt-2">We couldn't find this page. Explore topics below.</p>
        <ul className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
          {LANDINGS.map((l) => (
            <li key={l.slug}>
              <Link className="text-blue-600 hover:underline" to={`/l/${l.slug}`}>{l.h1}</Link>
            </li>
          ))}
        </ul>
      </div>
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: landing.h1, item: `/l/${landing.slug}` },
    ],
  };
  ...
  return (
    <div className="space-y-6">
      <section className="p-5 sm:p-8 bg-gradient-to-r from-amber-50 via-slate-50 to-blue-50 rounded-xl">
        <h1 className="text-3xl sm:text-4xl font-extrabold leading-tight">{landing.h1}</h1>
        <p className="text-slate-700 mt-2 text-sm sm:text-base">{landing.description}</p>
      </section>
      ...
      <section className="p-4 sm:p-6">
        ...
        <ul className="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-sm">
          {LANDINGS.filter((l) => l.slug !== landing.slug).slice(0, 8).map((l) => (
            <li key={l.slug}>
              <Link className="text-blue-600 hover:underline" to={`/l/${l.slug}`}>{l.h1}</Link>
            </li>
          ))}
